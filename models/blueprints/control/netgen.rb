module RockAUV
    module Control
        ...
        Producer = Struct.new :name, :domain, :axis, :srv
        ...
        def self.netgen(producers)
            ...
            producers.each do |name, raw_producer|
                resolved_producer = producer_domains(name, raw_producer)
            ...
            NETGEN_RULES.each do |rule|
                if resolved_producers = producers_by_domains[rule.source_domain]
                    new_axis, new_producer = apply_rule(result, rule, resolved_producers)
        ...
        def self.apply_rule(composition_m, rule, producers)
            ...
            return new_axis, convertion_child
        end
    end
end
